    ...
    <div class="chat-main">
      <template v-if="currentSession">
        ...
        <div class="chat-messages" ref="messagesContainer">
          ...
          <div v-else v-for="(message, index) in currentMessages" :key="index" class="message-wrapper">
...
// 导入API函数
import { getChatSessions, getChatMessages, sendChatMessage, markSessionRead, uploadChatFile, reportChatMessage } from '@/api/chat';
...
window.__chatEmergencyRecover = function() {
  try {
    ...
    // 如果有会话数据，选择第一个
    if (chat.sessions && chat.sessions.length > 0 && typeof chat.selectSession === 'function') {
      setTimeout(() => {
...
if (!window.__EMOJI_FIXED__) {
  ...
  window.EmojiPanel = window.EmojiPanel || { 
    init: function() { console.log('模拟表情面板初始化'); return null; }
  };
...
const installGlobalErrorRecovery = () => {
  ...
  window.addEventListener('error', (event) => {
    ...
    // 判断是否为渲染相关错误
    if (event.message?.includes('render') || 
...
const createEmergencyRefreshButton = () => {
  try {
    ...
    button.style.cssText = 'position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);' + 
...
const selectSession = async (session) => {
  ...
  // 防止重复选择相同会话
  if (currentSession.value && currentSession.value.id === session.id && !session.temporary) return;
...
const loadMessages = async (isLoadMore = false) => {
  ...
  console.log('开始加载消息，会话ID:', currentSession.value.id, '是否加载更多:', isLoadMore);
  ...
  try {
    ...
    if (res && typeof res === 'object' && res.error === 0 && res.body) {
      ...
      // 如果是虚拟会话，尝试获取会话详细信息
      if (currentSession.value && currentSession.value.name === '加载中...' && messages.length > 0) {
        // 从消息中获取会话信息
        const otherMsg = messages.find(m => m.senderId !== currentUserId.value);
        if (otherMsg) {
...
const sendTextMessage = async () => {
  ...
  try {
    ...
    if (res && typeof res === 'object' && res.error === 0 && res.body) {
      ...
      console.error('发送消息失败，API响应:', res);
