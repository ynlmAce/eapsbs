...
import { verifyUsername as checkUsername, verifySecurityAnswer as checkSecurityAnswer, resetPassword as resetPasswordAPI } from '@/api/user';
...
const verifyUsername = async () => {
  try {
    ...
    // 调用后端API验证用户名是否存在
    try {
      const apiResponse = await checkUsername(form.username);
      
      if (apiResponse.error === 0) {
        // 获取密保问题
        securityQuestion.value = apiResponse.body.securityQuestion;
        activeStep.value = 1;
      } else {
        ElMessage.error(apiResponse.message || '验证用户名失败');
      }
    } catch (error) {
...
const verifySecurityAnswer = async () => {
  try {
    ...
    // 调用后端API验证密保答案
    try {
      const apiResponse = await checkSecurityAnswer(form.username, form.securityAnswer);
      
      if (apiResponse.error === 0) {
        activeStep.value = 2;
      } else {
        ElMessage.error(apiResponse.message || '密保答案验证失败');
      }
    } catch (error) {
...
const resetPassword = async () => {
  try {
    ...
    // 调用后端API重置密码
    try {
      const apiResponse = await resetPasswordAPI(form.username, form.securityAnswer, form.newPassword);
      
      if (apiResponse.error === 0) {
        activeStep.value = 3;
      } else {
        ElMessage.error(apiResponse.message || '重置密码失败');
      }
    } catch (error) {
