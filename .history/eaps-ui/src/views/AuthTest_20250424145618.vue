<template>
  <div class="auth-test-container">
    ...
    <div class="actions">
      ...
      <el-button type="info" @click="testSession" :loading="loading.session">
        测试会话状态
      </el-button>
...
const testSession = async () => {
  ...
  try {
    ...
    const roleVal = localStorage.getItem('userRole') || ''
    
    // 收集会话状态
    const sessionState = {
      token: {
        exists: !!tokenVal,
        length: tokenVal.length,
        excerpt: tokenVal ? `${tokenVal.substring(0, 10)}...${tokenVal.substring(tokenVal.length - 5)}` : 'none'
      },
      role: {
        exists: !!roleVal,
        value: roleVal,
        normalized: roleVal.toLowerCase()
      },
      userInfo: {}
    }
    ...
    try {
      ...
      // 检查角色匹配
      if (roleVal && userInfo?.role) {
        sessionState.roleMatch = roleVal.toLowerCase() === userInfo.role.toLowerCase()
      }
    } catch (e) {
      sessionState.userInfo.parseError = e.message
    }
    
    // 显示结果
    response.value = JSON.stringify(sessionState, null, 2)
    
    // 检查会话状态并给出建议
    if (!tokenVal) {
      ElMessage.error('会话状态异常: 缺少token')
    } else if (!roleVal) {
      ElMessage.warning('会话状态异常: 缺少角色信息')
    } else {
      ElMessage.success('会话状态基本正常')
    }
  } catch (error) {
    response.value = `测试会话出错: ${error.message}`
    ElMessage.error('测试会话状态失败')
  } finally {
  ...
  if (userInfoStr) {
    try {
      ...
      if (userInfoObj.token && !localStorage.getItem('token')) {
        ...
        console.log('已修复: token已从userInfo中恢复')
      }
      
      // 2. 修复角色问题
      if (userInfoObj.role && !localStorage.getItem('userRole')) {
        // 确保角色小写存储，与路由守卫检查的格式一致
        const normalizedRole = userInfoObj.role.toLowerCase()
        localStorage.setItem('userRole', normalizedRole)
        userRole.value = normalizedRole
        console.log('已修复: userRole已从userInfo中恢复')
      }
      
      // 3. 验证token格式
      const tokenValue = localStorage.getItem('token')
      if (tokenValue && tokenValue.length < 20) {
        console.warn('警告: token可能格式不正确，长度太短')
        ElMessage.warning('token格式可能不正确，建议重新登录')
      }
      
      ElMessage.success('快速修复已应用')
    ...
    ElMessage.error('无法修复: 未找到用户信息，请重新登录')
    setTimeout(() => {
      window.location.href = '/login'
    }, 1500)
  }
