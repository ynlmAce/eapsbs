...
const contactCompany = async (application) => {
  console.log('联系企业，申请信息:', application);
  loadingContact.value = true;
  
  try {
    let companyUserId = application.companyId;
    
    // 如果没有直接的companyId，尝试通过岗位ID获取
    if (!companyUserId) {
      console.log('申请中没有企业ID，尝试通过岗位ID获取企业用户ID');
    ...
    console.log('准备创建聊天会话，企业用户ID:', companyUserId, '岗位ID:', application.jobId);
    ...
    console.log('创建聊天会话成功:', chatResponse);
    
    // 跳转到聊天页面
    router.push({
      name: 'chat',
      params: { sessionId: chatResponse.id }
    });
  } catch (error) {
