...
const contactCompany = async (application) => {
  try {
    ...
    // 获取企业ID - 如果application.companyId不存在，通过jobId获取企业用户ID
    let companyUserId;
    try {
      // 先尝试通过jobId获取企业用户ID
      ElMessage.info('正在获取企业信息...');
      companyUserId = await getCompanyUserIdByJobId(application.jobId);
      console.log('获取到企业用户ID:', companyUserId);
      
      if (!companyUserId) {
        ElMessage.error('无法获取企业信息，请联系管理员');
        return;
      }
    } catch (error) {
      console.error('获取企业用户ID失败:', error);
      ElMessage.error('无法获取企业用户信息，请稍后再试');
      return;
    }
    
    // 使用获取到的企业用户ID创建聊天会话
    const res = await createChatSession('SE', companyUserId, Number(application.jobId), '');
