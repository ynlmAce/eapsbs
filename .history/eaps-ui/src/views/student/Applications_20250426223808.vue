...
const contactCompany = async (application) => {
  ...
  try {
    ...
    if (!companyUserId) {
      console.error('无法获取企业用户ID');
      ElMessage.error('无法联系企业，请稍后再试');
      loadingContact.value = false;
      return;
    }
    
    console.log('准备创建会话，企业用户ID:', companyUserId, '岗位ID:', application.jobId);
