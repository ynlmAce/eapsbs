...
const contactCompany = async (application) => {
  try {
    console.log('申请信息:', application);
    let companyUserId = application.companyId;

    // 如果companyId不可用，则尝试通过jobId获取企业用户ID
    if (!companyUserId) {
      console.log('companyId不可用，尝试通过jobId获取企业用户ID');
      const loadingInstance = ElLoading.service({ fullscreen: true, text: '正在获取企业信息...' });
      ...
      try {
        ...
        if (!companyUserId) {
          ElMessage.error('无法获取企业联系信息，请稍后再试');
          return;
        }
        ...
        ElMessage.error('获取企业信息失败，请稍后再试');
        return;
      }
    }

    console.log('准备创建聊天会话，参数:', {
    ...
    ElMessage.error('创建聊天会话失败，请稍后再试');
  }
};
