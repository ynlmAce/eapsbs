...
import { createChatSession, createStudentToCompanyChat } from '@/api/chat';
...
const contactCompany = async (application) => {
  try {
    ...
    // 获取企业ID - 如果application.companyId不存在，尝试从后端获取
    let companyId = application.companyId;
    if (!companyId) {
      // 这里通常应该从应用记录中获取companyId，但如果不存在，可以尝试通过jobId来查询
      // 临时解决方案：使用一个默认值或从其他信息中提取
      ElMessage.warning('缺少企业ID信息，将使用临时值进行处理');
      companyId = application.jobId; // 临时使用jobId替代（实际使用时应从后端获取正确的companyId）
    }
    
    // 在请求中直接使用companyId作为targetUserId临时解决方案
    const res = await createChatSession('SE', Number(companyId), Number(application.jobId), '');
