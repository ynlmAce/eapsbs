  ...
  try {
    ...
    if (Array.isArray(dataList)) {
      applications.value = dataList.map(item => {
        // 进行字段映射，确保companyId字段存在
        return {
          id: item.applicationId,
          jobId: item.jobId,
          companyId: item.companyId || item.companyProfileId, // 尝试使用可能的备选字段
...
const contactCompany = async (application) => {
  ...
  console.log('开始联系企业，申请信息:', application);
  
  try {
    // 确保有企业用户ID
    let companyUserId = application.companyUserId;
    
    // 如果没有企业用户ID，尝试通过岗位ID获取
    if (!companyUserId) {
      console.log('申请中缺少企业用户ID，尝试通过岗位ID获取');
      try {
        companyUserId = await getCompanyUserIdByJobId(application.jobId);
        console.log('成功获取企业用户ID:', companyUserId);
      } catch (error) {
        console.error('获取企业用户ID失败:', error);
        ElMessage.error('无法获取企业联系信息，请稍后重试');
    ...
    if (!companyUserId) {
      console.error('无法获取有效的企业用户ID');
      ElMessage.error('无法获取企业联系信息，请联系管理员');
      loadingContact.value = false;
      return;
    }
    
    // 创建聊天会话
    console.log('创建聊天会话，企业用户ID:', companyUserId, '岗位ID:', application.jobId);
    const sessionResponse = await createChatSession({
      participantId: companyUserId,
      jobId: application.jobId,
    });
    
    console.log('聊天会话创建成功:', sessionResponse);
    
    // 跳转到聊天页面
    router.push({
      name: 'Chat',
      params: { sessionId: sessionResponse.sessionId }
    });
  } catch (error) {
    console.error('创建聊天会话失败:', error);
    ElMessage.error('联系企业失败，请稍后重试');
  } finally {
    loadingContact.value = false;
  }
};
...
// 根据岗位ID获取企业用户ID的辅助函数
const getCompanyUserIdByJobId = async (jobId) => {
  try {
    console.log('开始查询岗位信息，jobId:', jobId);
    const response = await getJobById(jobId);
    
    if (response && response.publisherId) {
      console.log('获取到岗位发布者ID:', response.publisherId);
      return response.publisherId; // 返回企业用户ID
    } else {
      console.error('岗位信息中未包含企业用户ID:', response);
      return null;
    }
  } catch (error) {
