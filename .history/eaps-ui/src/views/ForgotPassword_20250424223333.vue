...
const verifyUsername = async () => {
  try {
    ...
    // 调用后端API验证用户名是否存在
    try {
      const response = await axios.post('/api/user/verify-username', {
        username: form.username
      });
      
      if (response.data.error === 0) {
        // 获取密保问题
        securityQuestion.value = response.data.body.securityQuestion;
        activeStep.value = 1;
      } else {
        ElMessage.error(response.data.message || '验证用户名失败');
      }
    } catch (error) {
...
const verifySecurityAnswer = async () => {
  try {
    ...
    // 调用后端API验证密保答案
    try {
      const response = await axios.post('/api/user/verify-security-answer', {
        username: form.username,
      ...
      if (response.data.error === 0) {
        ...
        ElMessage.error(response.data.message || '密保答案验证失败');
      }
    } catch (error) {
