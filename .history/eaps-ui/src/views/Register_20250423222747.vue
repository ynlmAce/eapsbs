<template>
  <div class="register-container">
    <div class="register-box">
      ...
      <el-tabs v-model="activeRole" class="register-tabs" @tab-click="handleRoleChange">
        ...
        <el-tab-pane label="辅导员注册" name="counselor">
          <el-form
            ref="counselorForm"
            :model="counselorForm"
            ...
            label-position="top"
            class="register-form"
            @submit.prevent="submitCounselorForm"
          >
            <el-form-item prop="employeeId" label="工号">
              <el-input
                v-model="counselorForm.employeeId"
                placeholder="请输入工号"
                clearable
              />
            </el-form-item>
            ...
            <el-form-item prop="confirmPassword" label="确认密码">
              <el-input
                v-model="counselorForm.confirmPassword"
                type="password"
                placeholder="请再次输入密码"
                show-password
                clearable
...
const validateConfirmPassword = (rule, value, callback) => {
  ...
  } else if (activeRole.value === 'company') {
    if (value !== companyForm.password) {
      callback(new Error('两次输入密码不一致'))
    } else {
      callback()
    }
  } else if (activeRole.value === 'counselor') {
    if (value !== counselorForm.password) {
      callback(new Error('两次输入密码不一致'))
    } else {
      callback()
    }
...
// 监听路由参数，如果有role参数，切换到对应的角色
onMounted(() => {
  const { role } = route.query
  if (role && ['student', 'company', 'counselor'].includes(role)) {
...
const handleRoleChange = () => {
  // 切换角色时重置表单
  if (studentForm.value) studentForm.value.resetFields()
  if (companyForm.value) companyForm.value.resetFields()
  if (counselorForm.value) counselorForm.value.resetFields()
}
  ...
  await companyForm.value.validate(async (valid) => {
    if (valid) {
      try {
        ...
        const userData = {
          username: companyForm.creditCode,
          password: companyForm.password,
          role: 'company',
          profile: {
            name: companyForm.name,
            unified_social_credit_code: companyForm.creditCode,
            hr_contact: companyForm.contact,
...
// 辅导员注册表单提交
const submitCounselorForm = async () => {
  if (!counselorForm.value) return
  
  await counselorForm.value.validate(async (valid) => {
    if (valid) {
      try {
        loading.value = true
        
        const userData = {
          username: counselorForm.employeeId,
          password: counselorForm.password,
          role: 'counselor',
          profile: {
            name: counselorForm.name,
            contact: counselorForm.contact,
            employee_id: counselorForm.employeeId
          }
        }
